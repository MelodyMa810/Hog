...
def free_bacon(score):
    """ input: opponent score output: the number of points scored by rolling 0 dice. 
    ...
    ==> the nth digit of pi + 3 when current player rolls zero dice
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    take_turn(2, 0, make_test_dice(4, 5, 1)) == 9
    take_turn(3, 0, make_test_dice(4, 6, 1)) == 1 
    take_turn(0, 2) == 7
    take_turn(0, 0) == 6
    when num_rolls = 0 ==> nth digit (opponent_score) + 3, if not then just sum things up and not related to opponent_score at all
    """
...
def swine_align(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4a
    "*** YOUR CODE HERE ***"
    def GCD(score_one, score_two):
        i,gcd = 1,0
        while i <= min(score_one,score_two):
            if score_one % i ==0 and score_two % i ==0:
                gcd = i
            i += 1
        return gcd
...
def pig_pass(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4b
    "*** YOUR CODE HERE ***"
    difference = abs(opponent_score-player_score)
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    s0, s1 = hog.play(always(5), always(5), goal=10, dice=always_three) == 15,0
    s0, s1 = hog.play(always(5), always(3), score0=91, score1=10, dice=always_three) == 119,10
    s0, s1 = hog.play(strat0, strat1, score0=71, score1=80, dice=always_seven) == 83 (free_bacon). 108
    """
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    running_high = keep track of the highest jump
    last_score = previous score 
    """
    assert who == 0 or who == 1, 'The who argument should indicate a player.'
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    def find_highest(score0, score1):
        if who == 0:
            difference = score0 - last_score
            if difference > running_high:
                print(difference, 'point(s)! The most yet for Player 0')
                return announce_highest(who, score0, difference)
        ...
        if who == 1:
            ...
            if difference > running_high:
                ...
                return announce_highest(who, score1, difference)
...
def always_roll(n):
    ...
    both takes in a function as an argument and returns a function
    A strategy is a function that takes two total scores as arguments (the
    current player's score, and the opponent's score), and returns a number of
    dice that the current player will roll this turn.
...
def make_averaged(original_function, trials_count=1000):
    ...
    def average(*args):
        ...
        while n<=trials_count:
            single_outcome = original_function(*args)
            sum += single_outcome
            if n == trials_count:
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    result,n,max_value = 1,1,0
    while n<=10:
        new_value = make_averaged(roll_dice, trials_count=1000) (n, dice)
        if new_value > result:
            result = new_value
            max_value = n
        n += 1
